...
void mau(char *fc, char *bc="")
{
    ...
    if(strlen(bc)>0)
        for (y = 0; y < 16; y++)
            if(strcmpi(colors[y],fc)==0)
                break;
    mau(x, y);
}

void mau(char *fc, int bc)
{
    int x;
    char *colors[]={"Black","Blue","Green","Aqua","Red","Purple","Yellow","White","Gray",
    "LightBlue","LightGreen","LightAqua","LightRed","LightPurple","LightYellow","BrightWhite"};
...
void getup()
{
    ...
    printf("_",191);
    gotoxy(59,21);
    printf("  Diem  : 100  ",179,179);
    gotoxy(59,22);
    printf("  STATUS: Playing %c",179,179);
    gotoxy(58,23);
    printf("_",192);
    for (x = 0; x < 18; x++)
        printf("_",196);
    gotoxy(32,21);
    printf("Eat the food: o");
    gotoxy(32,22);
    printf("Snake Game");
}
